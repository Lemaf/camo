...
class Document extends BaseDocument {
    ...
    static findOne(query, options) {
        ...
        let projection = null,
            levelProjection = {};
        if (options && options.hasOwnProperty('projection')) {
            projection = options.projection;
            _.keys(projection).forEach(function(k){
                if(isObject(projection[k])){
                    levelProjection[k] = 1;
                }else{
                    levelProjection[k] = projection[k];
                }
            })
        }

        return DB().findOne(this.collectionName(), query, levelProjection)
        .then(function(data) {
            if (!data) {
                return null;
            }
            ...
            if (populate === true || (isArray(populate) && populate.length > 0) ||
                ...
                return that.populate(doc, populate, projection);
            }
    ...
    static find(query, options) {
        ...
        .then(function(datas) {
            ...
            if (options.populate === true ||
                ...
                return that.populate(docs, options.populate, options.projection);
            }
