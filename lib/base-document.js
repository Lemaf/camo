...
class BaseDocument {
     ...
     * @returns {Promise}
     */
    static populate(docs, fields, projection) {
        ...
        _.keys(anInstance._schema).forEach(function(key) {
            ...
            else if (isDocument(anInstance._schema[key].type)) {
                keys.push(key);
            }
        });
        ...
        keys.forEach(function(k) {
            ...
            documents.forEach(function(d) {
                ...
                if (isDocument(d._schema[k].type) && !isString(d[k]) && !DB().isNativeId(d[k]))
                    return;
        ...
        _.keys(ids).forEach(function(key) {
            ...
            let projectionConfig = projection ? projection[key] : {};

            // Bulk load dereferences
            let p = type.find({ '_id': { $in: keyIds } }, { populate: deepPopulate, projection: projectionConfig })
